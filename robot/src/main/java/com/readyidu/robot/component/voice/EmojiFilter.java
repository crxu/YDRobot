...
public class EmojiFilter implements InputFilter {
    ...
    private static void addUnicodeRangeToSet2(Set<String> set, int code, int start, int end) {
        if (set == null) {
            return;
        }
        for (int i = start; i <= end; i++) {
            filterSet.add(new String(new int[]{code, i}, 0, 2));
        }
    }
    ...
    static {
        ...
        addUnicodeRangeToSet(filterSet, 0X23f2);
        addUnicodeRangeToSet(filterSet, 0X23f1);
        addUnicodeRangeToSet(filterSet, 0X2328);
        addUnicodeRangeToSet(filterSet, 0X26ff);
        addUnicodeRangeToSet(filterSet, 0X27bf);
        addUnicodeRangeToSet(filterSet, 0X25b6);
        addUnicodeRangeToSet(filterSet, 0X23fa);
        addUnicodeRangeToSet(filterSet, 0X23ed);
        addUnicodeRangeToSet(filterSet, 0X23ee);
        addUnicodeRangeToSet(filterSet, 0X23f8);
        addUnicodeRangeToSet2(filterSet, 0Xd83e, 0xdd00, 0xdeFF);
        addUnicodeRangeToSet2(filterSet, 0Xd83d, 0xdd00, 0xdeFF);
    }
...
//        }
        if (filterSet.contains(source.toString())) {
            ToastUtil.showToast(YDRobot.getInstance().getContext(), "不支持Emoji表情！");
            return "";
        }
        return source;
    }
